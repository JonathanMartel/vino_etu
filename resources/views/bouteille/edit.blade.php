...
@if(isset($bouteille->code_saq) && isset(Auth::user()->admin))
<!-- La bouteille est importé de la saq ou créée par un administrateur -->
    ...
    <!-- L'utilisateur a droit de modifier une bouteille de la SAQ si c'est un admin -->
        ...
        <h1 class="titre-formulaire">Modifier une bouteille de la SAQ</h1>

        <!-- formulaire de modification d'une bouteille de la SAQ -->
        <form class="col s12 edit-vin" action="#" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
...
@elseif (Auth::user()->id === $bouteille->user_id || isset(Auth::user()->admin))
